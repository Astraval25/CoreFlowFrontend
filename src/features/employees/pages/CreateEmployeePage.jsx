  ...
  return (
    <div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 max-w-2xl space-y-4">
        {errors.submit && (
          <p className="text-xs p-3 rounded" style={{ color: "var(--red)", background: "rgba(239,68,68,0.08)" }}>{errors.submit}</p>
        )}

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Employee Code <span style={{ color: "var(--red)" }}>*</span></label>
            ...
            {errors.employeeCode && <p className="text-xs mt-1" style={{ color: "var(--red)" }}>{errors.employeeCode}</p>}
          </div>
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Employee Name <span style={{ color: "var(--red)" }}>*</span></label>
            <input name="employeeName" value={formData.employeeName} onChange={handleChange} className="form-input text-xs w-full" />
            {errors.employeeName && <p className="text-xs mt-1" style={{ color: "var(--red)" }}>{errors.employeeName}</p>}
          </div>
        </div>

        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Phone</label>
            <input name="phone" value={formData.phone} onChange={handleChange} className="form-input text-xs w-full" />
          </div>
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Email</label>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Designation</label>
            <input name="designation" value={formData.designation} onChange={handleChange} className="form-input text-xs w-full" />
          </div>
          <div>
            <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Joined Date <span style={{ color: "var(--red)" }}>*</span></label>
            <input name="joinedDt" type="date" value={formData.joinedDt} onChange={handleChange} className="form-input text-xs w-full" />
            {errors.joinedDt && <p className="text-xs mt-1" style={{ color: "var(--red)" }}>{errors.joinedDt}</p>}
          </div>
        </div>
        ...
        {!isEdit && (
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Salary Type</label>
            ...
            <div>
              <label className="text-xs font-medium mb-1 block" style={{ color: "var(--text-sub)" }}>Monthly Amount</label>
