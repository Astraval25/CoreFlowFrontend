...
const useCreatePaymentReceived = (paymentId = null) => {
  ...
  const fetchCustomers = async (compId) => {
    try {
      const res = await coreApi.getAllCustomerByCompanyId(compId);
      setAllCustomers(res?.data?.responseData || []);
    } catch (error) {
  ...
  const fetchUnpaidOrders = async (compId, customerId) => {
    if (!compId || !customerId) {
      setUnpaidOrders([]);
      return;
    }
    try {
      const res = await coreApi.getCustomerUnpaidOrders(compId, customerId);
  ...
  const fetchPaymentDetail = async (compId, pid) => {
    try {
      ...
      // For received payments, the customer info may come from buyerCompanyId or customerId
      const custId = data.customerId || data.buyerCompanyId;
      if (custId) {
        fetchUnpaidOrders(compId, custId);
      }
    } catch (error) {
  ...
  useEffect(() => {
    ...
    if (!companyId || !formData.customerId) return;
    fetchUnpaidOrders(companyId, formData.customerId);
  }, [companyId, formData.customerId, isEditMode]);
