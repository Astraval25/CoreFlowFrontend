...
const usePaymentReceivedPage = () => {
  ...
  const fetchPayments = async (compId) => {
    setLoading(true);
    try {
      const res = await coreApi.getPaymentsReceivedSummary(compId);
      setPayments(res?.data?.responseData || []);
    } catch (error) {
      console.error("Failed to fetch payment received summary:", error);
      setPayments([]);
    } finally {
      setLoading(false);
    }
...
export default usePaymentReceivedPage;
