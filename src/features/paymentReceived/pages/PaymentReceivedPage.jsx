            ...
            {!loading && filteredPayments.length === 0 ? (
              ...
              filteredPayments.map((p, index) => (
                <tr
                  ...
                  onClick={() =>
                    navigate(`/cf/company/${companyId}/payment-received/${p.paymentId}/detail`)
                  }
                >
                  ...
                  <td className="px-5 py-3 text-xs font-semibold text-[var(--accent)]">
                    {p.localPaymentNumber || p.paymentNumber || "-"}
                  </td>
                  <td className="px-5 py-3 text-xs text-[var(--text-sub)]">
                    {p.paymentDate ? new Date(p.paymentDate).toLocaleString() : "-"}
                  </td>
                  <td className="px-5 py-3 text-xs text-[var(--text-sub)]">{p.customerName || "-"}</td>
                  ...
                  <td className="px-5 py-3" onClick={(e) => e.stopPropagation()}>
                    <button
                      ...
                      onClick={() =>
                        navigate(`/cf/company/${companyId}/payment-received/${p.paymentId}/update`)
                      }
                    >
                      Edit
                    </button>
